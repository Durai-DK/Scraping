...
for r in range(2,l_ws.max_row+1):
    ...
    if l_ws.cell(row=r,column=7).value != "N/A":
        ws.cell(row=r,column=2).value = l_ws.cell(row=r,column=7).value

        print("#" * 120)
        print(" ")
        print("Flipkart")
        print('Range : ', r)
        print("Flipkart Url : ", l_ws.cell(row=r, column=7).value)

        try:
            driver.get(url=l_ws.cell(row=r, column=7).value)
            time.sleep(2)

            price = driver.find_element(By.CLASS_NAME, "_30jeq3").text
            print("Flipkart Price = ", price[1:])
            ws.cell(row=r, column=5).value = price[1:]

        except:
            pass

    wb.save(r"D:\Durai\Scraping\Mobile Appliance\Save Data\Scraping Files\Total Scraping\Mobile Flipkart " + date + ".xlsx")
